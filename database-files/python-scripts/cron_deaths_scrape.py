...
df.to_sql('times_report_deaths',schema = 'import', con =engine, if_exists='replace')

#loop through the states and update the data

for k, v in state.items():
	db_conn.execute("""INSERT INTO "REPORTED_DEATHS"	
	("STATE_ID","NUM_DEATHS","DATE_REPORTED")
	VALUES ( {},(SELECT SUM("{}") FROM import.times_report_deaths WHERE "Province_State" = '{}'),'{}')""".format(k,dString,v,dString,v))
